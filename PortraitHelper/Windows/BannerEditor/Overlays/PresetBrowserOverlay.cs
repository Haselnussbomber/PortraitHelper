...
public partial class PresetBrowserOverlay : Overlay
{
    ...
    private void DrawPresetBrowserContent()
    {
        ...
        ImGuiUtils.DrawSection(
            _textService.Translate("PresetBrowserOverlay.Sidebar.Presets.Title"),
            pushDown: false,
    ...
    private void DrawPresetCard(SavedBannerPreset preset, float scale, uint defaultImGuiTextColor)
    {
        ...
        if (ImGui.IsItemHovered() && !ImGui.IsMouseDragging(ImGuiMouseButton.Left))
        {
            using (ImRaii.Tooltip())
            {
                ...
                if (hasErrors)
                {
                    ...
                    {
                        ImGui.TextUnformatted(_textService.Translate("PresetBrowserOverlay.Tooltip.ElementsNotApplied.Title"));
                        ...
                        using (ImRaii.PushIndent(1))
                        {
                            if (!isBannerTimelineUnlocked)
                            {
                                ...
                                ImGui.SameLine(0, 5);
                                ImGui.TextUnformatted(_textService.Translate("PresetBrowserOverlay.Tooltip.ElementsNotApplied.PoseNotUnlocked", _bannerService.GetBannerTimelineName(preset.Preset!.BannerTimeline)));
                            }
                            ...
                            if (!isBannerBgUnlocked && _excelService.TryGetRow<BannerBg>(preset.Preset!.BannerBg, out var bannerBgRow))
                            {
                                ImGui.TextUnformatted("•");
                                ImGui.SameLine(0, 5);
                                ImGui.TextUnformatted(_textService.Translate("PresetBrowserOverlay.Tooltip.ElementsNotApplied.BackgroundNotUnlocked", bannerBgRow.Name.ExtractText()));
                            }
                            ...
                            if (!isBannerFrameUnlocked && hasBannerFrameRow)
                            {
                                ImGui.TextUnformatted("•");
                                ImGui.SameLine(0, 5);
                                ImGui.TextUnformatted(_textService.Translate("PresetBrowserOverlay.Tooltip.ElementsNotApplied.FrameNotUnlocked", bannerFrameRow.Name.ExtractText()));
                            }
                            ...
                            if (!isBannerDecorationUnlocked && hasBannerDecorationRow)
                            {
                                ImGui.TextUnformatted("•");
                                ImGui.SameLine(0, 5);
                                ImGui.TextUnformatted(_textService.Translate("PresetBrowserOverlay.Tooltip.ElementsNotApplied.DecorationNotUnlocked", bannerDecorationRow.Name.ExtractText()));
                            }
                        }
                    }
                }
        ...
        if (ImGui.MenuItem(_textService.Translate("PresetBrowserOverlay.ContextMenu.DeletePreset.Label")))
