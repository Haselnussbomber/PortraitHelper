    ...
    : Overlay(windowManager, textService, languageProvider, pluginConfig, excelService)
{
    ...
    public override void Draw()
    {
        ...
        if (ImGui.Button(textService.GetAddonText(14923) ?? "Select All"))
            ClipboardUtils.CurrentImportFlags = ImportFlags.All;

        ImGui.SameLine();

        if (ImGui.Button(textService.GetAddonText(14924) ?? "Deselect All"))
            ClipboardUtils.CurrentImportFlags = ImportFlags.None;

        ImGui.SameLine();

        if (ImGui.Button(textService.Translate("PortraitHelperWindows.AdvancedImportOverlay.ImportSelectedSettingsButton.Label")))
        {
            ClipboardUtils.ClipboardPreset.ToState(logger, bannerUtils, ClipboardUtils.CurrentImportFlags);
            MenuBar.CloseOverlays();
        }

        ImGuiUtils.DrawSection(textService.GetAddonText(14684) ?? "Design", respectUiTheme: !IsWindow);

        var isBannerBgUnlocked = bannerUtils.IsBannerBgUnlocked(ClipboardUtils.ClipboardPreset.BannerBg);
        DrawImportSetting(
            textService.GetAddonText(14687) ?? "Background",
            ImportFlags.BannerBg,
            () =>
            {
                if (ExcelService.TryGetRow<BannerBg>(ClipboardUtils.ClipboardPreset.BannerBg, out var bannerBgRow))
        ...
        var isBannerFrameUnlocked = bannerUtils.IsBannerFrameUnlocked(ClipboardUtils.ClipboardPreset.BannerFrame);
        DrawImportSetting(
            textService.GetAddonText(14688) ?? "Frame",
            ImportFlags.BannerFrame,
            () =>
            {
                if (ExcelService.TryGetRow<BannerFrame>(ClipboardUtils.ClipboardPreset.BannerFrame, out var bannerFrameRow))
        ...
        var isBannerDecorationUnlocked = bannerUtils.IsBannerDecorationUnlocked(ClipboardUtils.ClipboardPreset.BannerDecoration);
        DrawImportSetting(
            textService.GetAddonText(14689) ?? "Accent",
            ImportFlags.BannerDecoration,
            () =>
            {
                if (ExcelService.TryGetRow<BannerDecoration>(ClipboardUtils.ClipboardPreset.BannerDecoration, out var bannerDecorationRow))
        ...
        DrawImportSetting(
            ...
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.CameraZoom.ToString())
        );

        DrawImportSetting(
            textService.GetAddonText(14712) ?? "Rotation",
            ImportFlags.ImageRotation,
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.ImageRotation.ToString())
        );

        ImGuiUtils.DrawSection(textService.GetAddonText(14685) ?? "Character", respectUiTheme: !IsWindow);

        var isBannerTimelineUnlocked = bannerUtils.IsBannerTimelineUnlocked(ClipboardUtils.ClipboardPreset.BannerTimeline);
        DrawImportSetting(
            textService.GetAddonText(14690) ?? "Pose",
            ImportFlags.BannerTimeline,
            () =>
            {
                ImGui.TextUnformatted(bannerUtils.GetBannerTimelineName(ClipboardUtils.ClipboardPreset.BannerTimeline));
        ...
        DrawImportSetting(
            ...
            () =>
            {
                var id = ClipboardUtils.ClipboardPreset.Expression;
        ...
        DrawImportSetting(
            ...
            () => ImGui.TextUnformatted(textService.Translate("PortraitHelperWindows.Setting.AnimationTimestamp.ValueFormat", ClipboardUtils.ClipboardPreset.AnimationProgress))
        );

        DrawImportSetting(
            textService.GetAddonText(5972) ?? "Camera Position",
            ImportFlags.CameraPosition,
            () => DrawHalfVector4(ClipboardUtils.ClipboardPreset.CameraPosition)
        );

        DrawImportSetting(
            textService.Translate("PortraitHelperWindows.Setting.CameraTarget.Label"),
            ImportFlags.CameraTarget,
            () => DrawHalfVector4(ClipboardUtils.ClipboardPreset.CameraTarget)
        );

        DrawImportSetting(
            textService.Translate("PortraitHelperWindows.Setting.HeadDirection.Label"),
            ImportFlags.HeadDirection,
            () => DrawHalfVector2(ClipboardUtils.ClipboardPreset.HeadDirection)
        );

        DrawImportSetting(
            textService.Translate("PortraitHelperWindows.Setting.EyeDirection.Label"),
            ImportFlags.EyeDirection,
            () => DrawHalfVector2(ClipboardUtils.ClipboardPreset.EyeDirection)
        );
        ...
        DrawImportSetting(
            ...
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.AmbientLightingBrightness.ToString())
        );
        ...
        DrawImportSetting(
            ...
            () => DrawColor(
                ClipboardUtils.ClipboardPreset.AmbientLightingColorRed,
                ClipboardUtils.ClipboardPreset.AmbientLightingColorGreen,
                ClipboardUtils.ClipboardPreset.AmbientLightingColorBlue
            )
        );
        ...
        DrawImportSetting(
            ...
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.DirectionalLightingBrightness.ToString())
        );
        ...
        DrawImportSetting(
            ...
            () => DrawColor(
                ClipboardUtils.ClipboardPreset.DirectionalLightingColorRed,
                ClipboardUtils.ClipboardPreset.DirectionalLightingColorGreen,
                ClipboardUtils.ClipboardPreset.DirectionalLightingColorBlue
            )
        );
        ...
        DrawImportSetting(
            ...
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.DirectionalLightingVerticalAngle.ToString())
        );

        DrawImportSetting(
            textService.GetAddonText(14695) ?? "Horizontal Angle",
            ImportFlags.DirectionalLightingHorizontalAngle,
            () => ImGui.TextUnformatted(ClipboardUtils.ClipboardPreset.DirectionalLightingHorizontalAngle.ToString())
        );
    ...
    private static void DrawImportSetting(string label, ImportFlags flag, System.Action drawFn, bool isUnlocked = true)
    {
        ...
        var isEnabled = isUnlocked && ClipboardUtils.CurrentImportFlags.HasFlag(flag);
        ...
        {
            if (isEnabled)
                ClipboardUtils.CurrentImportFlags |= flag;
            else
                ClipboardUtils.CurrentImportFlags &= ~flag;
        }
